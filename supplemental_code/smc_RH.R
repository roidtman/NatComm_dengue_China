...
load('../output/prior_beta_rh.RData')
...
calc_prop_beta = function(tt, biv.temp, biv.mosq, biv.rh, b0.spline.tmp, browse = F){
  if(browse) browser()
  ...
  param.prop.new = c(mosq.coefs, T.coefs, rh.coefs, b0.spline.tmp[tt])
...
ll = function(params, browse = F){
  ...
  rh.biv = generate_biv_spline(coef.prop = params[19:27], dat.range = rh.range)
  
  beta.ts = c(rep(0, length(weights.serialinterval)), 
              exp(sapply((length(weights.serialinterval) + 1):nrow(GZ.data), function(tt)
    calc_prop_beta(tt, biv.temp = temp.biv, biv.mosq = mosq.biv, biv.rh = rh.biv,
...
f = paste0('../output/posterior_SMC_V3_rh_', setDate, '_', count, '.RData')
save(out, file = f)
