...
for(ii in 1:3){
  f = paste0('../output/posterior_SMC_V3_constant_beta0_20180517_', ii, '.RData')
  load(f)
  if(ii == 1){
...
calc_prop_beta = function(tt, biv.temp, biv.mosq, b0.spline.tmp, browse = F){
  ...
  param.prop.new = c(mosq.coefs, T.coefs, b0.spline.tmp)
...
if(!file.exists(f)){
  beta_mat = matrix(NA, nrow = nrow(param.chain), ncol = nrow(GZ.data))
  for(ii in 1:nrow(param.chain)){
    mosq.biv = biv.coef.arr$mosq.coefs[,,ii]
    temp.biv = biv.coef.arr$temp.coefs[,,ii]
    b0.spline = int.chain[ii]
    
    beta_mat[ii,] = c(rep(0, length(weights.serialinterval)), 
                      exp(sapply((length(weights.serialinterval) + 1):nrow(GZ.data), function(tt)
...
f = paste0('../output/fig_modelVal_constant_beta0.pdf')
...
text(x = length(cases_agg) * 0.92, y = 4.8, labels = expression(rho* ' = -0.007'))
...
f = paste0('../output/fig_modelVal_red-blue_constant_beta0.pdf')
...
# =============================================================== # 
## contribution to beta figure 

# calculating contributions to beta
mosq.arr = biv.coef.arr$mosq.coefs
temp.arr = biv.coef.arr$temp.coefs
mat = matrix(0, nrow = length(int.chain), ncol = nrow(GZ.data))
lagEnd = length(l) + 1
newdata.in_keep = array(0, c(length(lagEnd:nrow(GZ.data)), (1+length(l)*2), length(int.chain)))
for(ll in 1:length(int.chain)){
  print(ll)
  ll.tmp = 1
  if(ll %% 10 == 0){
    newdata.in_keep[, 1, ll/10] = int.chain[ll]
    newdata.in_keep[, 2:50, ll/10] = mosq.arr[lagEnd:nrow(GZ.data), , ll]
    newdata.in_keep[, 51:99, ll/10] = temp.arr[lagEnd:nrow(GZ.data), , ll]
    tmp = sapply(1:nrow(newdata.in_keep), function(ii) sum(newdata.in_keep[ii,, ll/10], na.rm = T))
    mat[ll.tmp, ] = c(rep(0, lagEnd - 1), exp(tmp))
    ll.tmp = ll.tmp + 1
  }
} 


beta_b0 = newdata.in_keep[,1,1:100]
beta_b0 = beta_b0[1,]
beta_mosq = newdata.in_keep[,2:50,1:100]
beta_temp = newdata.in_keep[,51:99,1:100]

f = paste0('../output/contribution_to_beta_constant_beta0.pdf')
...
b0_CI = matrix(nrow = 3, ncol = nrow(GZ.data))

b0_CI[1, ] = rep(quantile(beta_b0, probs = 0.0275))
b0_CI[2, ] = rep(quantile(beta_b0, probs = 0.975))
b0_CI[3, ] = rep(quantile(beta_b0, probs = 0.5))

# b0_CI=write_CI(beta_b0)
...
for(ii in 1:5){
  ...
  lines(rep(beta_b0[ii], nrow(GZ.data)), col = adjustcolor(col = col.vec[ii], alpha.f = 0.8), lwd = 1.5)
}
lines(b0_CI[3,], lwd = 2, col = rgb(0,0,0,0.7))
abline(v = seq(from = 1, to = 4017, by = 366), lwd = 1, col = rgb(0,0,0,0.3))
text(x=65, y = max(b0_CI)*0.9, labels = 'B', cex = 2)
mtext(text = expression(''*beta[0]), side = 2, line = 3.2, cex = 0.7)

# calculate the lines for mosq and temp and precip contributions
mosq.cont = matrix(NA, nrow = dim(beta_mosq)[3], ncol = nrow(GZ.data))
...
temp.cont = matrix(NA, nrow = dim(beta_temp)[3], ncol = nrow(GZ.data))
...
# mosq
plot(-100,-100, xlab = '', ylab = '', xlim = c(0,4017), ylim = c(min(mosq.cont_CI), 
                                                                 max(mosq.cont_CI)), 
     xaxt = 'n', xaxs = 'i', las = 1)
...
abline(v = seq(from = 1, to = 4017, by = 366), lwd = 1, col = rgb(0,0,0,0.3))
text(x=65, y = max(mosq.cont_CI, na.rm = T) * 0.9, labels = 'C', cex = 2)
mtext(text = expression(sum(s[m](m(t-tau), tau), tau = 1, 49)), side =2 , line = 1.95, cex = 0.7)

# temp
ind = 1:3987
plot(-100,-100, xlab = '', ylab = '', xlim = c(0,4017), ylim = c(min(temp.cont_CI, na.rm = T), 
                                                                 max(temp.cont_CI, na.rm = T)), 
     xaxt = 'n', xaxs = 'i', las = 1)
...
text(x=65, y = max(temp.cont_CI, na.rm = T) * 0.9, labels = 'D', cex = 2)
