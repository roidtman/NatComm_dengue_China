...
load('../output/prior_beta_precip.RData')
...
calc_prop_beta = function(tt, biv.temp, biv.mosq, biv.precip, b0.spline.tmp, browse = F){
  if(browse) browser()
  ...
  param.prop.new = c(mosq.coefs, T.coefs, precip.coefs, b0.spline.tmp[tt])
...
ll = function(params, browse = F){
  ...
  b0.spline = generate_spline(y.in = params[28:length(params)], bs.in = b0_basis)
...
# prior function
p = function(params){
  p1 = dmvnorm(params[1:27], param.mean[1:27], makePositiveDefinite(sigma.mean), log = TRUE)
  p2 = dnorm(mean(params[28:length(initial.param)]), 0, 2, log = TRUE)
...
s = function(n1=1, n2=33){
  # browser()
  s1 = rmvnorm(n1, param.mean[1:27], makePositiveDefinite(sigma.mean))
  s2 = rnorm(n2, 0, 0.5)
  return(c(s1, s2))
}
...
f = paste0('../output/posterior_SMC_V3_precip_', setDate, '_', count, '.RData')
save(out, file = f)
