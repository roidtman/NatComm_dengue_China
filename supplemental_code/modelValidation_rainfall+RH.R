...
for(ii in 1:3){
  f = paste0('../output/posterior_SMC_V3_precip_rh_20180807_', ii, '.RData')
  load(f)
  if(ii == 1){
...
hum.biv.chain = param.chain[, 28:36]
int.chain = param.chain[, 37:ncol(param.chain)]
...
# inputs for bivariate humidity spline 
humidity = GZ.data$RH_mean

rh.range = seq(min(humidity), max(humidity), by = 1)
rh.range.round = signif(rh.range, 2)
l = 1:49

align_index_rh = function(dat = GZ.data, browse = F){
  if(browse) browser()
  rh.ind = matrix(NA, nrow = nrow(dat), ncol = length(l))
  
  for(ii in (length(l) + 1): nrow(dat)){
    rh = dat$RH_mean[ii - l]
    tmp = unlist(sapply(1:length(l), function(kk) which(rh.range.round == rh[kk])))
    if(length(tmp) == length(l)){
      rh.ind[ii, ] = tmp
    }else{
      rh.ind[ii, ] = c(tmp, rep(NA, (length(l) - length(tmp))))
    }
  }
  return(rh.ind)
}
rh.ind = align_index_rh(browse = F)
...
generate_coef = function(m.biv.chain, t.biv.chain, p.biv.chain, rh.biv.chain, dat = GZ.data, browse = F){
  if(browse) browser()
  ...
  rh.coefs = array(NA, dim = c(nrow(dat), length(l), nrow(rh.biv.chain)))
  ...
  for(ll in 1:nrow(m.biv.chain)){
    ...
    biv.rh = generate_biv_spline(coef.prop = rh.biv.chain[ll, ], dat.range = rh.range, lag.dat = l)
    ...
    for(tt in (length(l) + 1):nrow(dat)){
      ...
      precip.tmp = unlist(sapply(1:length(l), function(ii) biv.precip[precip.ind[tt, ii], l[ii]]))
      precip.coefs[tt, , ll] = precip.tmp
      
      rh.tmp = unlist(sapply(1:length(l), function(ii) biv.rh[rh.ind[tt, ii], l[ii]]))
      rh.coefs[tt, , ll] = rh.tmp
    } 
  }
  
  list.coef = list(mosq.coefs, temp.coefs, precip.coefs, rh.coefs)
  names(list.coef) = c('mosq.coefs', 'temp.coefs', 'precip.coefs', 'rh.coefs')
  return(list.coef)
}
...
write_beta = function(int.spline.mat, biv.arr, dat = GZ.data, browse = F, extra = F){
  ...
  if(!extra){
    ...
    rh.arr = biv.arr$rh.coefs
    
    mat = matrix(0, nrow = nrow(int.spline.mat), ncol = nrow(dat))
    lagEnd = length(l) + 1
    newdata.in = matrix(0, nrow = length(lagEnd:nrow(dat)), ncol = (1 + length(l) * 4))
    ...
    for(ll in 1:nrow(int.spline.mat)){
      ...
      newdata.in[, 149:197] = rh.arr[lagEnd:nrow(dat), , ll]
    ...
    mat = matrix(0, nrow = nrow(int.spline.mat), ncol = nrow(dat))
    lagEnd = length(l) + 1
    newdata.in_keep = array(0, c(length(lagEnd:nrow(dat)), (1+length(l)*4), nrow(int.spline.mat)/10))
    newdata.in_dontKeep = matrix(0, nrow = length(lagEnd:nrow(dat)), ncol = (1 + length(l) * 4))
    ...
    for(ll in 1:nrow(int.spline.mat)){
      ...
      if(ll %% 10 == 0){
        ...
        newdata.in_keep[, 149:197, ll/10] = rh.arr[lagEnd:nrow(dat), , ll]
        ...
        newdata.in_dontKeep[, 149:197] = rh.arr[lagEnd:nrow(dat), , ll]
...
if(!file.exists(f)){
  biv.coef.arr = generate_coef(m.biv.chain = mosq.biv.chain, t.biv.chain = weather.biv.chain, 
                               p.biv.chain = precip.biv.chain, rh.biv.chain = hum.biv.chain, browse = F)
...
f = paste0('../output/fig_modelVal_RH_rainfall.pdf')
...
text(x = length(cases_agg) * 0.92, y = 4.8, labels = expression(rho* ' = 0.912'))
...
f = paste0('../output/fig_modelVal_red-blue_RH_rainfall.pdf')
...
f = '../output/contribution_to_beta_rainfall_RH.RData'
if(!file.exists(f)){
...
beta_RH = beta_extra[[2]][,149:197,]

f = paste0('../output/contribution_to_beta_rainfall_RH.pdf')
pdf(f, height = 8, width = 8)
...
abline(v = seq(from = 1, to = 4017, by = 366), lwd = 1, col = rgb(0,0,0,0.3))
text(x=65, y = max(beta_b0)*0.8, labels = 'B', cex = 2)
...
RH.cont = matrix(NA, nrow = 100, ncol = nrow(GZ.data))
for(ii in 1:dim(beta_RH)[3]){
  tmp = sapply(1:nrow(beta_RH), function(kk) sum(beta_RH[kk,,ii]))
  RH.cont[ii, ] = c(rep(0, 49), (tmp - mean(tmp, na.rm = T)))
}
RH.cont_CI = write_CI(RH.cont)
...
text(x=65, y = max(precip.cont, na.rm = T) * 0.9, labels = 'E', cex = 2)

# RH
ind = 1:3987
plot(-100,-100, xlab = '', ylab = '', xlim = c(0,4017), ylim = c(min(RH.cont, na.rm = T), 
                                                                 max(RH.cont, na.rm = T)), 
     xaxt = 'n', xaxs = 'i', las = 1)
polygon(c(time[ind], rev(time[ind])), 
        c(RH.cont_CI[1, ind], rev(RH.cont_CI[2,ind])), density = NA, 
        col = rgb(0,0,1,0.3),
        border = NA)
for(ii in 1:5){
  lines(RH.cont[ii,], col = adjustcolor(col = col.vec[ii], alpha.f = 0.8), lwd = 1.5)
}
lines(RH.cont_CI[3,], lwd = 2, col = rgb(0,0,0,0.7))
abline(v = seq(from = 1, to = 4017, by = 366), lwd = 1, col = rgb(0,0,0,0.3))
mtext(text = expression(sum(s[RH](RH(t-tau), tau), tau = 1, 49)), side =2 , line = 1.95, cex = 0.7)
text(x=65, y = max(RH.cont, na.rm = T) * 0.9, labels = 'F', cex = 2)
