...
ll = function(params, browse = F){
  ...
  beta.ts = c(rep(0, length(weights.serialinterval)), exp(sapply((length(weights.serialinterval) + 1):nrow(GZ.data), 
                                                                 function(tt)
...
p = function(params){
  ...
  p2 = dnorm(params[19], 0, 2, log = TRUE)
  return(0.001 * (sum(p1) + sum(p2)))
}
# sampler function
s = function(n1=1, n2=1){
  # browser()
...
f = paste0('../output/posterior_SMC_V3_', setDate, '_', count, '.RData')
save(out, file = f)
