...
load('../output/prior_beta_20180807.RData')
...
calc_prop_beta = function(tt, biv.temp, biv.mosq, biv.precip, biv.rh, b0.spline.tmp, browse = F){
  if(browse) browser()
  ...
  precip.coefs = unlist(sapply(1:length(l), function(ii) biv.precip[precip.ind[tt, ii], l[ii]]))
  rh.coefs = unlist(sapply(1:length(l), function(ii) biv.rh[rh.ind[tt, ii], l[ii]]))
  
  param.prop.new = c(mosq.coefs, T.coefs, precip.coefs, rh.coefs, b0.spline.tmp[tt])
...
ll = function(params, browse = F){
  ...
  b0.spline = generate_spline(y.in = params[37:length(params)], bs.in = b0_basis)
  ...
  precip.biv = generate_biv_spline(coef.prop = params[19:27], dat.range = precip.range)
  rh.biv = generate_biv_spline(coef.prop = params[28:36], dat.range = rh.range)
  
  beta.ts = c(rep(0, length(weights.serialinterval)), 
              exp(sapply((length(weights.serialinterval) + 1):nrow(GZ.data), function(tt)
    calc_prop_beta(tt, biv.temp = temp.biv, biv.mosq = mosq.biv, 
                   biv.precip = precip.biv, biv.rh = rh.biv,
                   browse = F, b0.spline.tmp = b0.spline))))
...
# prior function
p = function(params){
  p1 = dmvnorm(params[1:36], param.mean[1:36], makePositiveDefinite(sigma.mean), log = TRUE)
  p2 = dnorm(mean(params[37:69]), 0, 2, log = TRUE)
...
s = function(n1=1, n2=33){
  # browser()
  s1 = rmvnorm(n1, param.mean[1:36], makePositiveDefinite(sigma.mean))
  s2 = rnorm(n2, 0, 0.5)
  return(c(s1, s2))
}
...
f = paste0('../output/posterior_SMC_V3_precip_rh_', setDate, '_', count, '.RData')
save(out, file = f)
